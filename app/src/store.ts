...
export class Store {
  ...
  ui = { symbol: "BTC-USD", theme: "dark" as "dark" | "light", onboarded: false, livePref: true };
  ...
  async enableLive(): Promise<void> {
    ...
    // Fetch real candles; failures are silent — synthetic data remains.
    await Promise.allSettled(
      LIVE_SYMBOLS.map(async (sym) => {
        try {
          const bars = await fetchCandles(sym, "1h", 500);
  ...
  private async pollLive() {
    ...
    await Promise.allSettled(
      LIVE_SYMBOLS.map(async (sym) => {
        try {
          const bars = await fetchCandles(sym, "1h", 5);
