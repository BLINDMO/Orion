...
function renderTfPicker() {
  ...
  wrap.querySelectorAll("[data-tf]").forEach((b) => b.onclick = () => {
...
function renderIndicatorPicker() {
  ...
  const items = [["sma", "SMA"], ["ema", "EMA"], ["bb", "BB"], ["vwap", "VWAP"]];
  wrap.innerHTML = items.map(([k, l]) => `<button class="chip tf ${ind[k] ? "active" : ""}" data-ind="${k}">${l}</button>`).join("");
  wrap.querySelectorAll("[data-ind]").forEach((b) => b.onclick = () => {
    ...
    <div class="panel">
      ...
      <div class="field"><label>Order type</label>
        <select class="input" id="typeSel">
          ${["market", "limit", "stop", "stop-limit", "trailing-stop"].map((t) => `<option value="${t}" ${t === state.form.type ? "selected" : ""}>${t}</option>`).join("")}
        </select></div>
...
function updatePreview() {
  ...
  preview.innerHTML = `
    ...
    <div class="row"><span class="k">After (buy)</span><span class="num ${after < 0 ? "loss" : ""}">${money(after)}</span></div>`;
...
function renderScanPanel() {
  ...
  panel.innerHTML = `
    <div class="scan">
      ...
      ${s.readings.filter((r) => r.value !== void 0).slice(0, 6).map((r) => `
        <div class="reading"><span class="muted">${r.indicator}</span>
          <span class="num">${typeof r.value === "number" ? Math.abs(r.value) > 100 ? compact(r.value) : r.value.toFixed(2) : "\u2014"} <span class="dim">${r.state}</span></span></div>`).join("")}
...
function updateBook() {
  ...
  if (positions) positions.innerHTML = pos.length === 0 ? `<div class="empty">No open positions.</div>` : pos.map((p) => {
    ...
    return `<div class="li"><div><div class="sym">${label}</div><div class="dim num">${qty(p.qty)} @ ${priceFmt(p.avgCost.toNumber())}</div></div>
        <div class="right"><div class="num">${priceFmt(mark)}</div><div class="num ${pnlClass(upnl)}">${glyph(upnl)} ${money(upnl, { sign: true })}</div></div>
        <button class="x" data-close="${p.key}" title="Close">\u2297</button></div>`;
  }).join("");
  positions?.querySelectorAll("[data-close]").forEach((b) => b.onclick = () => closePosition(b.dataset.close));
  const orders = document.getElementById("orders");
  const wo = W().workingOrders();
  if (orders) orders.innerHTML = wo.length === 0 ? `<div class="empty">No working orders.</div>` : wo.map((o) => `<div class="li"><div><div class="sym">${o.side.toUpperCase()} ${qty(o.qty - o.filledQty)} ${(o.target.option?.underlying ?? o.target.symbol).replace("-USD", "")}</div>
      <div class="dim">${o.type}${o.limitPrice ? " @ " + priceFmt(o.limitPrice) : ""}${o.stopPrice ? " stop " + priceFmt(o.stopPrice) : ""}</div></div>
      <button class="x" data-cancel="${o.id}">\u2715</button></div>`).join("");
  orders?.querySelectorAll("[data-cancel]").forEach((b) => b.onclick = () => {
    store.cancel(b.dataset.cancel);
    refresh();
  });
  const blotter = document.getElementById("blotter");
  const fills = W().fills.slice(-8).reverse();
  if (blotter) blotter.innerHTML = fills.length === 0 ? `<div class="empty">No fills yet.</div>` : fills.map((f) => `<div class="li"><div><div class="sym">${f.side.toUpperCase()} ${qty(f.qty)} ${(f.target.option?.underlying ?? f.target.symbol).replace("-USD", "")}</div>
      <div class="dim num">${dateLabel(f.at)}</div></div>
      <div class="right"><div class="num">${priceFmt(f.price.toNumber())}</div>${!f.realized.isZero() ? `<div class="num ${pnlClass(f.realized.toNumber())}">${money(f.realized.toNumber(), { sign: true })}</div>` : `<div class="dim num">fee ${money(f.fee.toNumber())}</div>`}</div></div>`).join("");
}
...
function animateAdvance(from, target) {
  ...
  function frame(now) {
    ...
    if (inter > W().now) W().advanceTo(inter, { stepRes: target - from > 2 * 864e5 ? "1h" : "1m" });
...
function updateHeader() {
  ...
  if (acct) {
    ...
    acct.innerHTML = `
      <div class="kv"><span class="k">Equity</span><span class="v num">${money(eq)}</span></div>
      <div class="kv"><span class="k">Cash</span><span class="v num">${money(cash)}</span></div>
      <div class="kv"><span class="k">Buying Pwr</span><span class="v num">${money(bp)}</span></div>
      <div class="kv"><span class="k">Total P&L</span><span class="v num ${pnlClass(totalPnl)}">${glyph(totalPnl)} ${money(totalPnl, { sign: true })}</span></div>`;
  }
  const now = document.getElementById("nowLabel");
  if (now) now.textContent = `\u25F7 ${dateLabel(W().now)} \xB7 ${W().mode === "live" ? "LIVE" : "SIM"}`;
  ...
  if (tag && bars.length) {
    ...
    tag.innerHTML = `<div class="px num">${priceFmt(last.c)}</div><div class="chg num ${pnlClass(chg)}">${glyph(chg)} ${pct(chg)}</div>`;
  }
}
function refresh() {
  updateHeader();
...
function renderNav() {
  ...
  nav.innerHTML = items.filter(([k]) => k !== "learn" || showLearn).map(([k, l, ic]) => `<button class="${state.screen === k ? "on" : ""}" data-nav="${k}"><span class="ic">${ic}</span>${l}</button>`).join("");
  nav.querySelectorAll("[data-nav]").forEach((b) => b.onclick = () => openScreen(b.dataset.nav));
}
...
function renderOptionsScreen() {
  ...
  const body = `
    <div class="card"><div class="row-between"><div><div class="muted">${sym()} spot</div><div class="num" style="font-size:22px;font-weight:700">${priceFmt(p.spot)}</div></div>
      <div class="dim" style="font-size:12px;text-align:right">${inst().assetClass === "equity" ? "American \xB7 physically settled" : "European \xB7 cash settled"}<br>tap a bid/ask to trade 1 contract</div></div>
    <div class="chain-tabs">${chain.expiries.map((e, i) => `<button class="chip ${i === state.optExpiryIdx ? "active" : ""}" data-exp="${i}">${dayLabel(e.expiry)}</button>`).join("")}</div>
...
function buildStrategy(name, exp, spot) {
  ...
  out.innerHTML = `
    <div class="card">
      <div class="row-between"><strong>${strat.name}</strong><span class="num ${a.netCost > 0 ? "loss" : "gain"}">${a.netCost > 0 ? "Debit" : "Credit"} ${money(Math.abs(a.netCost))}</span></div>
      ...
      <div class="muted mt" style="font-size:13px">Breakevens: ${a.breakevens.map((b) => priceFmt(b)).join(", ") || "\u2014"}</div>
...
function renderStatsScreen() {
  ...
  const body = `
    <div class="card"><div class="muted">Total equity</div><div class="num" style="font-size:30px;font-weight:700">${money(r.equity)}</div>
      <div class="num ${pnlClass(totalPnl)}">${glyph(totalPnl)} ${money(totalPnl, { sign: true })} total P&L</div>
    ...
    <div class="stat-grid">
      ...
      ${stat("Theta captured/paid", money(Number(r.options.thetaCapturedOrPaid)), pnlClass(Number(r.options.thetaCapturedOrPaid)))}
    ...
    ${r.byAssetClass.length ? `<h3 style="margin:20px 0 8px;color:var(--text-3);font-size:11px;letter-spacing:.1em;text-transform:uppercase">By asset class</h3>
      <div class="card">${r.byAssetClass.map((b) => `<div class="row-between" style="padding:6px 0"><span>${b.key}</span><span class="num ${pnlClass(Number(b.realized))}">${money(Number(b.realized), { sign: true })} <span class="dim">${b.trades} trades</span></span></div>`).join("")}</div>` : ""}`;
...
function renderLearnScreen() {
  const overlay = document.getElementById("overlay");
  if (!getCurriculum(W())) {
    overlay.innerHTML = screenShell("Learn", `<div class="empty">Help is disabled. Enable Help in Settings to access the learning track.</div>`);
    document.getElementById("backBtn").onclick = closeScreen;
    return;
  }
  ...
  const body = `<div class="muted" style="margin-bottom:16px">Options, end to end \u2014 ${(store.progress.overallProgress() * 100).toFixed(0)}% complete.</div>
    ...
    return `<div class="module ${st.unlocked ? "" : "locked"}" ${st.unlocked ? `data-mod="${m.id}"` : ""}>
        <div class="mhead"><strong>${m.index}. ${m.title}</strong>
          ${st.passed ? `<span class="badge done">\u2713 ${(st.bestScore * 100).toFixed(0)}%</span>` : st.unlocked ? "" : `<span class="badge">\u{1F512} locked</span>`}
...
function renderSettingsScreen() {
  ...
  const body = `
    ...
    <div class="card">
      <div class="set-row"><div class="label"><div>Bankroll top-up</div><div class="sub">Add simulated cash. Current: ${money(W().portfolio.cash.toNumber())}</div></div></div>
  ...
  document.getElementById("rfr").addEventListener("change", (e) => store.updateSettings({ riskFreeRate: (parseFloat(e.target.value) || 4) / 100 }));
  ...
  document.getElementById("resetBtn").onclick = () => {
    if (confirm("Reset the simulator? This wipes your portfolio, orders and clock.")) {
      store.reset();
      state.screen = "trade";
      renderShell();
